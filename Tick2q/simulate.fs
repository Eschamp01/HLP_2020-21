module Simulate
    ...
    module MapEnvt =
        ...
        let varLookup (vName: string) (envt:Environment) : Result<Wire,string> =
            ...
            | None -> Error "No variable exists with this name"
            | Some x -> x.Value
        ...
        let updateEnvt (vName: string) (newValue: Result<Wire,string>) (env: Environment) =
            let newVariable = {(Map.find vName env) with Value = newValue}
            let newEnv = Map.remove vName env
            Map.add vName newVariable newEnv
    ...
    let wAnd (f1: Update) (f2:Update) : Update =
        fun env ->
            ...
            | (Ok Zero, Ok One) -> Ok Zero
            | (Ok One, Ok Zero) -> Ok Zero
            | (Ok One, Ok One) -> Ok One
            | (Error _, Ok Zero) -> Ok Zero
            | (Ok Zero, Error _) -> Ok Zero
            | (Error x, Ok One) -> Error x
            | (Ok One, Error x) -> Error x
            | (_, _) -> Error "other AND error"
    ...
    let wOr (f1: Update) (f2:Update) : Update =
        fun env ->
            match (f1 env, f2 env) with
            | (Ok Zero, Ok Zero) -> Ok Zero
            | (Ok Zero, Ok One) -> Ok One
            | (Ok One, Ok Zero) -> Ok One
            | (Ok One, Ok One) -> Ok One
            | (Error x, Ok Zero) -> Error x
            | (Ok Zero, Error x) -> Error x
            | (Error _, Ok One) -> Ok One
            | (Ok One, Error _) -> Ok One
            | (_, _) -> Error "other OR error"
